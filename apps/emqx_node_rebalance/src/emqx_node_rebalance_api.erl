...
-rest_api(#{name   => load_rebalance_global_status,
            ...
            func   => global_status,
            descr  => "Get status of all rebalance/evacuation process across the cluster"}).
...
-rest_api(#{name   => load_rebalance_stop,
            ...
            func   => rebalance_stop,
            descr  => "Start rebalancing coordinated by the node"}).
...
rebalance_evacuation_stop(#{node := NodeBin}, _Params) ->
    validated(
      fun() ->
              {_, [Node]} = validate_nodes(node, [NodeBin]),
...
rpc(Node, M, F, A) ->
    case rpc:call(Node, M, F, A) of
        ...
        {badrpc, _} ->
            return({error, 400, io_lib:format("Error communicating node ~p", [Node])});
        Unknown ->
...
validate_evacuation(Node, Params) ->
    {_, [NodeToEvacuate]} = validate_nodes(node, [Node]),
    OptList = lists:map(
...
validate_rebalance(Node, Params) ->
    {_, [CoordinatorNode]} = validate_nodes(node, [Node]),
    OptList = lists:map(
