...
handle_ack(State, MessageId, Ack) ->
    ?tp(debug, handle_ack, #{message_id => MessageId, ack => Ack}),
    State.

handle_need_data(#{buffer := Buffer0} = State, DesiredCount) ->
    case is_buffer_empty(Buffer0) of
        true ->
            ?tp(debug, handle_next_buffer_empty, #{}),
            {ok, State#{wants_data => true}};
        false ->
            ?tp(debug, handle_next_buffer_not_empty, #{buffer_length => buffer_length(Buffer0)}),
...
handle_info(#{wants_data := true, buffer := Buffer0} = State, #fake_msg{} = Msg) ->
    ?tp(debug, handle_info_wants_data_true, #{buffer_length => buffer_length(Buffer0), msg => Msg}),
...
handle_info(#{wants_data := false, buffer := Buffer0} = State, #fake_msg{} = Msg) ->
    ?tp(debug, handle_info_wants_data_false, #{buffer_length => buffer_length(Buffer0), msg => Msg}),
    Buffer = buffer_in(Buffer0, make_messages(State, Msg)),
    {ok, State#{buffer => Buffer}};
handle_info(State, Info) ->
    ?tp(debug, handle_info_unknown, #{info => Info, state => State}),
    {ok, State}.
