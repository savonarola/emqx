...
-spec tools_to_openai_spec([tool_spec()]) -> openai_spec().
tools_to_openai_spec(Tools) ->
    lists:map(fun tool_to_openai_spec/1, Tools).

%%--------------------------------------------------------------------
%% Internal functions
%%--------------------------------------------------------------------

tool_to_openai_spec(#{name := Name, description := Description, parameters := Parameters}) ->
    #{
        type => <<"function">>,
        function => #{
            name => Name,
